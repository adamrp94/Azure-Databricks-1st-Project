...
1. [Initial Setup on Azure](#1-initial-setup-on-azure)
2. [Workspace Navigation](#2-workspace-navigation)
3. [Project Steps + Medallion Architecture](#3-project-steps--medallion-architecture)
4. [TPCH Dataset](#4-tpch-dataset--sample-data)
5. [Dimensional Model — Fact & Dim Tables](#5-dimensional-model--fact--dim-tables)
6. [Jobs and Pipelines](#6-jobs-and-pipelines)
7. [Dashboard in Databricks](#7-dashboard-in-databricks)
8. [Power BI Connection](#8-power-bi-connection)
9. [AI/ML — AutoML Forecasting](#9-aiml--automl-forecasting)
10. [LLM — AI Chat over the Data](#10-llm--ai-chat-over-the-data)
---
...
| Feature | Azure Databricks | Community Edition |
|---|---|---|
| Azure Active Directory | ✅ | ❌ |
| Storage Accounts | ✅ | ❌ |
| Unity Catalog | ✅ | ❌ |
| Azure Clusters | ✅ | ❌ |
| Power BI AAD connection | ✅ | ❌ |
| Enterprise security / VNET | ✅ | ❌ |
...
| **Catalog** | Heart of data governance — catalogs, schemas and tables (Bronze/Silver/Gold) |
...
| **Playground (AI/ML)** | Sandbox for experimenting with models, prompts and agents |
...
The project follows the **Medallion Architecture (Bronze → Silver → Gold)**, the professional standard for organising a Lakehouse in Databricks.

```
RAW Files  →  Bronze (raw tables)  →  Silver (cleaned)  →  Gold (analytics-ready)  →  Power BI
```
 
**Steps:**
 
1. **Create the catalog and schema (Unity Catalog)** — defines where tables will live (main catalog → bronze / silver / gold schemas)
2. **Create the Storage / External Location** — physical location for RAW data (Azure auto-provisions this)
3. **Load RAW data (Bronze)** — upload datasets in CSV, JSON or Parquet to the Storage Account
4. **Transform to Silver (cleaning)** — use SQL or PySpark to clean, type-cast and normalise
5. **Create Gold tables (analytics-ready)** — ready for Power BI or Fabric
6. **Connect Power BI** — using the native Azure Databricks connector with AAD authentication
...
Using SQL, `orders_cleaned` is created from `orders_raw` applying quality filters:
 
- ❌ Orders with total price `≤ 0` are removed
- ✅ Only orders with status `F` (Fulfilled), `O` (Open) or `P` (Pending) are kept
